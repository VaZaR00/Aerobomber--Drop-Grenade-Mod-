...
#define REMOTE_CALLCLASS(className,member,args,access) (if(isNil "_oopOriginCall")then{ [[_classID, member, SAFE_VAR(args),access], {_this call GETCLASS(className)}] remoteExecCall ["call", TARGET_VAR, DO_JIP] }else{ [[_oopOriginCall, [_classID, member, SAFE_VAR(args),access]], {(_this select 1) call GETCLASS((_this select 0))}] remoteExecCall ["call", TARGET_VAR, DO_JIP]})
#define REMOTE_SPAWNCLASS(className,member,args,access) (if(isNil "_oopOriginCall")then{ [[_classID, member, SAFE_VAR(args),access], {_this call GETCLASS(className)}] remoteExec ["call", TARGET_VAR, DO_JIP] }else{ [[_oopOriginCall, [_classID, member, SAFE_VAR(args),access]], {(_this select 1) call GETCLASS((_this select 0))}] remoteExec ["call", TARGET_VAR, DO_JIP]})
